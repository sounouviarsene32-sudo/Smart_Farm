...
const HealthEventService = {
  ...
  async updateHealthEvent(eventId, data) {
    const event = await HealthEvent.findByIdAndUpdate(eventId, data, {
      new: true,
    });
    return event;
  },
  ...
  async deleteHealthEvent(eventId) {
    ...
    if (animal) {
      animal.evenements = animal.evenements.filter(
  ...
  async analyzeAnimalHealth(animalId) {
    ...
    if (
      !dernierVaccin ||
      (now - new Date(dernierVaccin.date)) / (1000 * 60 * 60 * 24) > 30
    ) {
      await createAlert({
        type: "vaccination",
  ...
  async obtenirPlanningVaccins(idCampagne) {
    return await VaccineSchedule.find({ campaignId: idCampagne }).sort({
      scheduledDate: 1,
    });
  },
};
