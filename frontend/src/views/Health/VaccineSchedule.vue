<script setup>
import { 
  CheckCircle2, Heart, ShieldAlert, AlertCircle, 
  Syringe, CalendarPlus, UserCircle 
...
const vaccinations = [
  { id: 1, disease: 'Newcastle', status: 'À venir', dept: 'Volaille', animalCount: 450, date: '25 mars 2026', daysRemaining: 4, bgIcon: 'bg-orange-50', colorIcon: 'text-orange-500', badgeClass: 'bg-orange-500 text-white' },
  { id: 2, disease: 'Fièvre aphteuse', status: 'À venir', dept: 'Bovins', animalCount: 320, date: '28 mars 2026', daysRemaining: 7, bgIcon: 'bg-orange-50', colorIcon: 'text-orange-500', badgeClass: 'bg-orange-500 text-white' },
  { id: 3, disease: 'PPR', status: 'Planifié', dept: 'Caprins', animalCount: 280, date: '5 avril 2026', daysRemaining: 15, bgIcon: 'bg-blue-50', colorIcon: 'text-blue-500', badgeClass: 'bg-blue-600 text-white' },
  { id: 4, disease: 'Gumboro', status: 'Urgent', dept: 'Volaille', animalCount: 450, date: '22 mars 2026', daysRemaining: 1, bgIcon: 'bg-rose-50', colorIcon: 'text-rose-500', badgeClass: 'bg-rose-500 text-white' },
];

const recentTreatments = [
  { id: 1, date: '18 mars 2026', dept: 'Volaille', type: 'Antiparasitaire', count: 50, vet: 'Dr. Sow', status: 'Complété' },
  { id: 2, date: '17 mars 2026', dept: 'Bovins', type: 'Antibiotique', count: 12, vet: 'Dr. Diop', status: 'Complété' },
  { id: 3, date: '16 mars 2026', dept: 'Caprins', type: 'Anti-inflammatoire', count: 8, vet: 'Dr. Sow', status: 'Complété' },
  { id: 4, date: '15 mars 2026', dept: 'Ovins', type: 'Désinfection', count: 200, vet: 'Dr. Ndiaye', status: 'Complété' },
];
</script>
...
<template>
  <main class="flex-1 lg:ml-64 p-4 lg:p-8 transition-all duration-300 w-full bg-slate-50 min-h-screen space-y-8">
    ...
    <div class="grid my-10 grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div v-for="stat in healthStats" :key="stat.label" class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex justify-between items-center">
        <div>
    ...
    <div class="bg-white my-10 rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      ...
      <div class="divide-y divide-slate-50">
        <div v-for="vax in vaccinations" :key="vax.id" class="p-6 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
          <div class="flex items-center gap-4">
            ...
            <div>
              <div class="flex items-center gap-2">
                ...
                <span :class="['text-[10px] px-2 py-0.5 rounded font-bold uppercase', vax.badgeClass]">
                  {{ vax.status }}
                </span>
              </div>
      ...
      <table class="w-full text-left text-sm">
        ...
        <tbody class="divide-y divide-slate-50">
          <tr v-for="t in recentTreatments" :key="t.id" class="text-slate-600">
            ...
            <td class="px-6 py-4">
              <span class="bg-slate-100 px-2.5 py-1 rounded text-[11px] font-bold text-slate-600">{{ t.dept }}</span>
            </td>
            <td class="px-6 py-4">
               <div class="flex items-center gap-2">
            ...
            <td class="px-6 py-4">
              <div class="flex justify-center">
                <span class="bg-emerald-500 text-white px-3 py-1 rounded-full text-[10px] font-bold uppercase">
                  {{ t.status }}
                </span>
              </div>
            </td>
    ...
    <div class="bg-rose-50 my-10 border border-rose-100 p-5 rounded-2xl flex gap-4 items-start">
      ...
      <div>
        ...
        <p class="text-rose-700 text-sm mt-1">
          2 animaux en état critique nécessitent une attention immédiate. Veuillez contacter le vétérinaire.
        </p>
      </div>
    </div>
</main>
