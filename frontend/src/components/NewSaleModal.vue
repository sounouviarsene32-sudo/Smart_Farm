...
const validate = () => {
  ...
  if (form.value.quantity < 1)
    e.quantity = 'La quantité doit être suppérieur ou égale à 1'
  if (form.value.quantity > animals.value.length)
    e.quantity = 'Stock insuffisant'
...
const handleSubmit = async () => {  
  if (!validate()) {
    toast.warning('Formulaire de vente invalide ou stock insuffisant')
    return;
  }
  ...
  try {
    ...
    const response = await AnimalService.saleAnimals(payload.product, payload.quantity);
...
async function getAnimals() {
  const res = await AnimalService.getAnimalsByCampaign(campaignId)
  animals.value = res.filter(a => a.status === 'actif')
  breed.value = await AnimalService.breedAnimals();
}
  ...
  <Teleport to="body">
    <Transition name="modal">
      ...
      >
        <!-- Carte modale -->
        <div
          class="bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-slate-100 overflow-hidden"
        >
          <!-- En-tête -->
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-[11px] text-slate-400">
                  Remplis les champs pour enregistrer la vente.
                </p>
              </div>
            </div>
            <button
          ...
          <div class="p-6 space-y-4 max-h-[65vh] overflow-y-auto">
            ...
            <div>
              ...
              <input
                ...
                :class="[
                  'w-full p-2.5 bg-slate-50 border rounded-xl text-sm outline-none transition-all',
                  errors.clientName
                    ? 'border-red-400 focus:ring-2 focus:ring-red-300'
                    : 'border-slate-200 focus:ring-2 focus:ring-slate-950',
                ]"
              />
              <p v-if="errors.clientName" class="text-red-500 text-[11px] mt-1">
                {{ errors.clientName }}
              </p>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                >
              <option value="" disabled>Race choisie</option>
              <option v-for="b in breed" :value="b">{{ b }}</option>
              </select>
                <p v-if="errors.product" class="text-red-500 text-[11px] mt-1">
                  {{ errors.product }}
                </p>
              </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  :max="animals?.length"
                  :class="[
                    'w-full p-2.5 bg-slate-50 border rounded-xl text-sm outline-none',
                    errors.quantity
                      ? 'border-red-400'
                      : 'border-slate-200 focus:ring-2 focus:ring-slate-950',
                  ]"
                />
                <p v-if="errors.quantity" class="text-red-500 text-[11px] mt-1">
                  {{ errors.quantity }}
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p v-if="errors.unitPrice" class="text-red-500 text-[11px] mt-1">
                  {{ errors.unitPrice }}
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="flex gap-3">
                <label
                  v-for="opt in ['Payé', 'En attente']"
                  :key="opt"
                  :class="[
                    'flex-1 flex items-center justify-center gap-2 p-2.5 rounded-xl border text-sm font-semibold cursor-pointer transition-all',
                    form.status === opt
                      ? opt === 'Payé'
            ...
            <!-- Aperçu du total -->
            <div
              class="bg-blue-600 text-white rounded-xl px-5 py-3 flex justify-between items-center"
            >
