  ...
  return [
    {
      ...
      value: `${totalAmount.toLocaleString('fr-FR')} F CFA`,
      icon: DollarSign,
...
const salesByBreed = computed(() => {
  const depts = {}
  recentTransactions.value.forEach((tx) => {
    const amount = Number(tx.total.replace(/[^0-9.-]+/g, ''))
    depts[tx.product] = (depts[tx.product] || 0) + amount
  })
...
const pieData = computed(() => ({
  labels: salesByBreed.value.map((d) => d.name),
  datasets: [
    {
      data: salesByBreed.value.map((d) => d.percent),
      backgroundColor: salesByBreed.value.map((d) => d.color),
      borderWidth: 0,
      hoverOffset: 10,
    },
  ],
...
onMounted(async () => {
  try {
    ...
    const sales = salesRes.data.filter(campaignId => campaignId == campaignId)
    console.log(campaignId)
    console.log(sales)
    recentTransactions.value = sales.map((sale) => ({
      id: sale._id,
...
const filteredTransactions = computed(() => {
  ...
  return recentTransactions.value.filter(
    (tx) => tx.client.toLowerCase().includes(query) || tx.product.toLowerCase().includes(query),
  )
})
</script>

<template>
  <main class="flex-1 lg:p-8 transition-all duration-300 w-full bg-slate-50 min-h-screen space-y-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <h3 class="font-bold text-slate-800 mb-8 text-lg">Ventes par race</h3>
        <div class="flex items-center gap-8 flex-col md:flex-row">
          ...
          <div class="flex-1 space-y-4 w-full">
            <div
              v-for="(item, index) in salesByBreed"
              :key="item.name"
        ...
        <tbody class="divide-y divide-slate-50 font-medium">
          ...
          >
            ...
            <td class="px-6 py-4 text-center">
              <span
                :class="[
                  tx.status === 'Payé' ? 'bg-emerald-500 text-white' : 'bg-orange-500 text-white',
